...
# Build a path inside the general models directory
def model_path(*parts):
    return os.path.join(MODELS_DIR, *parts)

# Build a path inside the FLAN-T5 results directory
...
# Patch SetFit's model-card ID inference to fall back safely on a local model, avoiding a metadata bug
