...
EMAIL_RE    = r'\b\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+\b'
EMAIL_RE2 = r'mail\s*(?:is)?\s*:?\s*(.+)$'



# Return the substring starting right after the "password" marker to the end of the text
...
# Check whether the given text is a strong password (matches PASSWORD_RE)
...
# Return the substring starting right after the "mail" marker up to the password
...
# Check whether the given text is a valid email address (matches EMAIL_RE)
...
# Normalize a spoken-word email/password by mapping word tokens (e.g. "capital a") to characters until it passes the given validity check
...
# Extract and normalize an email address from spoken-word text, or "None" if invalid
...
# Extract and normalize a password from spoken-word text, or "None" if not strong enough
...
# Extract a username from text using the USERNAME_RE pattern, or "None" if not found
...
# Extract the mail, password and username from raw text into a 3-element list
...
# Extract mail/password/username from text and log the user in with whichever identifier was found
...
#PASSWORD_RE = r'\s(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[_@$!%*?&]).{6,}\s'
#PASSWORD_RE1 = r'password\s*(?:is|:)?\s*(\S+)'
#PASSWORD_RE2 = r'password\s*(?:is|:)?\s*(\S+)(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[_@$!%*?&]).{6,}\s'
